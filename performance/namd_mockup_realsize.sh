...
if [[ -f $i_pdb &&`stat --dereference --format=%s $i_pdb` -ne 8388608 ]]; then
    echo "ERROR: pdb file has wrong size!"
    exit 1
else
    echo "INFO: pdb file has correct size!"
fi
if [[ -f $i_crd && `stat --dereference --format=%s $i_crd` -ne 12582912 ]]; then
    echo "ERROR: crd file has wrong size!"
    exit 1
else
    echo "INFO: crd file has correct size!"
fi
if [[ -f $i_parm && `stat --dereference --format=%s $i_parm` -ne 14680064 ]]; then
...
if [[ -f $i_vel && `stat --dereference --format=%s $i_vel` -ne 2097152 ]]; then
    echo "ERROR: vel file has wrong size!"
    exit 1
else
    echo "INFO: vel file has correct size!"
fi
if [[ -f $i_xsc && `stat --dereference --format=%s $i_xsc` -ne 4194304 ]]; then
...
dd if=/dev/urandom of=$o_vel bs=1M count=2
ret=$?
if [ $ret -gt 0 ]; then
    echo "Writing output vel failed ...."
    exit $ret
fi
dd if=/dev/urandom of=$o_xsc bs=1M count=4
ret=$?
if [ $ret -gt 0 ]; then
...
# o_log_du = 167M
dd if=/dev/urandom of=$o_out bs=1M count=34
ret=$?
if [ $ret -gt 0 ]; then
    echo "Writing output out failed ...."
    exit $ret
fi
dd if=/dev/urandom of=$o_err bs=1M count=133
ret=$?
if [ $ret -gt 0 ]; then
...
# o_ana_du = 3.6G
dd if=/dev/urandom of=$o_dcd bs=1M count=1100
ret=$?
if [ $ret -gt 0 ]; then
    echo "Writing output dcd failed ...."
    exit $ret
fi
dd if=/dev/urandom of=$o_cvd bs=1M count=1200
ret=$?
if [ $ret -gt 0 ]; then
    echo "Writing output cvd failed ...."
    exit $ret
fi
dd if=/dev/urandom of=$o_xst bs=1M count=1300
ret=$?
if [ $ret -gt 0 ]; then
